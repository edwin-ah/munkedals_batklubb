<x-app-layout>
  <div class="container mx-auto mt-5">
    <h1 class="text-center text-2xl font-semibold text-main work-sans">Radera medlemsmötesprotokoll</h1>
    <div class=" mt-10 pt-8 pb-10 bg-white rounded shadow-xl">
      <div class="p-10">
        <div class="mb-4">
          <h1 class="text-lg font-semibold text-main work-sans">Är du säker på att du vill radera protokollet '{{ $protocol->title }}'?</h1>
        </div>
        <div x-data="{ btnDisabled: true }">

          <form action="{{ route('protocol.delete') }}" method="POST">
            @csrf
          ...
          <div class="mb-4">
            <a href="{{ route('protocol.index') }}" class="text-center rounded px-5 py-2 shadow btn-primary">Tillbaka</a>
          </div>
        </div>
      </div>
    </div>
